...
def del_items(request):
    ...
    return HttpResponse("OK 200 Item deleted {}".format(item_name))
...
def view_items(request):
    ...
    #find top 5 bids
    return HttpResponse("OK 200 Item Listed: {}".format(item))
...
def add_bid(request):
    if request.method == "POST":
        ...
        return HttpResponse("Added Item: {} {}".format(bid.item, bid.bid_amount))
...
def del_bids(request):
    ...
    return HttpResponse("OK 200 Bid deleted {} for ".format(item_name))


def view_bids(request):
    bidder=request.REQUEST['name']
    bid_list = bids.objects.filter(name = bidder)
    return HttpResponse("OK 200 \nBids Listed: {}".format(bid_list))
